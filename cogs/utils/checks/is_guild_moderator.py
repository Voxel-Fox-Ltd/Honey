...
class NotGuildModerator(commands.CommandError):
    ...
    def __init__(self, guild_moderator_role):
        self.guild_moderator_role = guild_moderator_role.name or "None"
        self.default_permission = "manage_message"
        message = 'You need the `{0}` permission or the guild moderator role ({1.name})!'.format(self.default_permission, guild_moderator_role)
